        ...
        $(document).on('click', '.js-follow-question', function (oEvent) {
            ...
            Action[bFollow ? 'unFollowQuestion' : 'followQuestion']({
                ...
                call: function (oResult) {
                    ...
                    // 修改数量
                    oCountEl.html(oResult.obj.fansCount + "人关注");
                    console.log(oResult);
                    if (bFollow) {
                        // 移除用户
                        oListEl.find('.js-user-' + oResult.obj.userId).remove();
                    } else {
                        // 显示用户
                        oListEl.prepend('<a class="zm-item-link-avatar js-user-' + oResult.obj.follower.id + '" href="/user/' + oResult.obj.follower.id + '" data-original_title="'
                            + oResult.obj.follower.name + '"><img src="' + oResult.obj.follower.profile + '" class="zm-item-img-avatar"></a>');
                    }
                },
                error: function (oResult) {
                    alert('出现错误，请重试');
